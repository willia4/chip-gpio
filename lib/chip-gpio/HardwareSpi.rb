...
require 'chip-gpio'
begin
  latch = ChipGPIO.get_pins()[:XIO1]
  latch.export if !latch.available? 
  latch.value = 0 

  words = Array.new(24) { 0 }
  spi = ChipGPIO::HardwareSPI.new(word_size: 12, lsb_first: false)

  r = spi.transfer_data(words: words.reverse())
  puts "#{r}"
